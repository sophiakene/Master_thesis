...
from sklearn.model_selection import train_test_split, TimeSeriesSplit, cross_val_score, StratifiedKFold, LeaveOneGroupOut
...
with h5py.File(data, 'r') as f:
    ...
    aggregated_data = np.mean(features, axis=1) #this aggregates all the channels into 1 mean signal. let's not do this now, let's try the vectorizer but still use all of the channels separately
    ...
    """for train_index, test_index in train_test_split(aggregated_data):
        ...
        "\n----------------------\n----------------------")
    """
    print("LEAVE-ONE-PARTICIPANT-OUT-VALIDATION OF SONG PREDICTION USING PRECOMPUTED ICA:")
    baseline_classifier = svm.SVC(kernel = "linear", C=0.0001)
    logo = LeaveOneGroupOut()
    cv = logo.split(aggregated_data, labels, subjects)
    print("number of splits: ",logo.get_n_splits(groups = subjects)) #9, good
    cv_scores_accuracy = cross_val_score(baseline_classifier, aggregated_data, labels, cv=cv, scoring='accuracy')
    ...
    """for i, (train_index, test_index) in enumerate(logo.split(aggregated_data, labels, subjects)):
        print(f"Fold {i}:")
        print(f"  Train: index={train_index}, group={subjects[train_index]}")
        print(f"  Test:  index={test_index}, group={subjects[test_index]}")"""
        #just for testing, this makes sense, testing on P01 in first fold usw.
    ...
    print("LEAVE-ONE-PARTICIPANT-OUT CROSS-VALIDATION OF METER PREDICTION USING PRECOMPUTED ICA:")
    baseline_classifier = svm.SVC(kernel = "linear", C=0.0001)
    cv = logo.split(aggregated_data, meter_labels, subjects)
    ...
    indices_to_remove = np.where(np.isin(labels, instrumental_ids))[0]

    filtered_features_wo_instrumental = np.delete(aggregated_data, indices_to_remove, axis=0)
    filtered_labels_wo_instrumental = np.delete(labels, indices_to_remove)
    filtered_subjects_wo_instrumental = np.delete(subjects, indices_to_remove)
    ...
    #print(len(lyrics_labels), len(filtered_features_wo_instrumental)) #both 360
    ...
    print("LEAVE-ONE-PARTICIPANT-OUT CROSS-VALIDATION OF LYRICS PREDICTION USING PRECOMPUTED ICA:")
    baseline_classifier = svm.SVC(kernel = "linear", C=0.0001)
    cv = logo.split(filtered_features_wo_instrumental, lyrics_labels, filtered_subjects_wo_instrumental)
