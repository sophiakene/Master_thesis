...
from sklearn.model_selection import train_test_split, TimeSeriesSplit, cross_val_score, StratifiedKFold, KFold
...
data = "FEATURES_DATASET_PERCEPTION2.h5"
...
with h5py.File(data, 'r') as f:
    ...
    print("HERE: ", Counter(filtered_labels.tolist()))
    ...
    cv_scores_accuracy = cross_val_score(baseline_classifier, filtered_features, filtered_labels, cv=cv, scoring='accuracy')
    cv_scores_precision = cross_val_score(baseline_classifier, filtered_features, filtered_labels, cv=cv, scoring='precision_macro')
    cv_scores_recall = cross_val_score(baseline_classifier, filtered_features, filtered_labels, cv=cv, scoring='recall_macro')
    cv_scores_f1 = cross_val_score(baseline_classifier, filtered_features, filtered_labels, cv=cv, scoring='f1_macro')
    print("\nACCURACY: ",cv_scores_accuracy, 
    "\nPRECISION: ", cv_scores_precision,
    "\nRECALL: ", cv_scores_recall,
    "\nF1SCORE: ", cv_scores_f1)
    y_pred = cross_val_predict(baseline_classifier, filtered_features, filtered_labels, cv=cv)
    ...
    cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    cv = KFold(n_splits=9, shuffle=False)
    cv_scores_accuracy = cross_val_score(baseline_classifier, filtered_features, meter_labels, cv=cv, scoring='accuracy')
    cv_scores_precision = cross_val_score(baseline_classifier, filtered_features, meter_labels, cv=cv, scoring='precision_macro')
    cv_scores_recall = cross_val_score(baseline_classifier, filtered_features, meter_labels, cv=cv, scoring='recall_macro')
    cv_scores_f1 = cross_val_score(baseline_classifier, filtered_features, meter_labels, cv=cv, scoring='f1_macro')
    print("\nACCURACY: ",cv_scores_accuracy, 
    "\nPRECISION: ", cv_scores_precision,
    "\nRECALL: ", cv_scores_recall,
    "\nF1SCORE: ", cv_scores_f1)
    y_pred = cross_val_predict(baseline_classifier, filtered_features, meter_labels, cv=cv)
    ...
    cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    cv = KFold(n_splits=9, shuffle=False)
    cv_scores_accuracy = cross_val_score(baseline_classifier, filtered_features, mode_labels, cv=cv, scoring='accuracy')
    cv_scores_precision = cross_val_score(baseline_classifier, filtered_features, mode_labels, cv=cv, scoring='precision_macro')
    cv_scores_recall = cross_val_score(baseline_classifier, filtered_features, mode_labels, cv=cv, scoring='recall_macro')
    cv_scores_f1 = cross_val_score(baseline_classifier, filtered_features, mode_labels, cv=cv, scoring='f1_macro')
    print("\nACCURACY: ",cv_scores_accuracy, 
    "\nPRECISION: ", cv_scores_precision,
    "\nRECALL: ", cv_scores_recall,
    "\nF1SCORE: ", cv_scores_f1)
    y_pred = cross_val_predict(baseline_classifier, filtered_features, mode_labels, cv=cv)
    ...
    filtered_features_wo_instrumental = np.delete(filtered_features, indices_to_remove, axis=0)
    ...
    filtered_features2 = np.delete(filtered_features, indices_to_remove, axis=0)
