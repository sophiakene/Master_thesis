...
with h5py.File(data, 'r') as f:
    ...
    cv_scores_precision = cross_val_score(baseline_classifier, X, filtered_labels, cv=cv, scoring='precision_macro')
    cv_scores_recall = cross_val_score(baseline_classifier, X, filtered_labels, cv=cv, scoring='recall_macro')
    cv_scores_f1 = cross_val_score(baseline_classifier, X, filtered_labels, cv=cv, scoring='f1_macro')
    ...
    baseline_classifier = svm.SVC(kernel = "linear", C=0.0001)
    cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    cv_scores_accuracy = cross_val_score(baseline_classifier, X, meter_labels, cv=cv, scoring='accuracy')
    cv_scores_precision = cross_val_score(baseline_classifier, X, meter_labels, cv=cv, scoring='precision_macro')
    cv_scores_recall = cross_val_score(baseline_classifier, X, meter_labels, cv=cv, scoring='recall_macro')
    cv_scores_f1 = cross_val_score(baseline_classifier, X, meter_labels, cv=cv, scoring='f1_macro')
    print("\nACCURACY: ",cv_scores_accuracy, 
    "\nPRECISION: ", cv_scores_precision,
    "\nRECALL: ", cv_scores_recall,
    "\nF1SCORE: ", cv_scores_f1)
    ...
    baseline_classifier = svm.SVC(kernel = "linear", C=0.0001)
    cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    cv_scores_accuracy = cross_val_score(baseline_classifier, X, mode_labels, cv=cv, scoring='accuracy')
    cv_scores_precision = cross_val_score(baseline_classifier, X, mode_labels, cv=cv, scoring='precision_macro')
    cv_scores_recall = cross_val_score(baseline_classifier, X, mode_labels, cv=cv, scoring='recall_macro')
    cv_scores_f1 = cross_val_score(baseline_classifier, X, mode_labels, cv=cv, scoring='f1_macro')
    print("\nACCURACY: ",cv_scores_accuracy, 
    "\nPRECISION: ", cv_scores_precision,
    "\nRECALL: ", cv_scores_recall,
    "\nF1SCORE: ", cv_scores_f1)
    y_pred = cross_val_predict(baseline_classifier, X, mode_labels, cv=cv)
    ...
    filtered_features_wo_instrumental = np.delete(X, indices_to_remove, axis=0)
    ...
    filtered_features2 = np.delete(X, indices_to_remove, axis=0)
